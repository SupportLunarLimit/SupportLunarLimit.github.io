...
const storage = {
  ...
  // ----- Robust event helpers -----
  function __when(cond, fn, checkEvery=100, timeout=8000){
    if (cond()) return void fn();
  ...
  function __bindDiag(){
    ...
    function setEnabled(ok){ btn.disabled = !ok; btn.textContent = ok ? label : 'Run (auth not ready)'; }
    ...
    btn.addEventListener('click', async ()=>{
      ...
      try{
        ...
        if (!f){ out.textContent = 'error: firebase not ready'; return; }
        ...
        document.getElementById('diagErr').textContent = msg;
        out.textContent = 'error: ' + msg;
      }
    });
  }
  function __attachDonationsListener(cb){
    const diagL = document.getElementById('diagListen'); if (diagL) diagL.textContent='attaching';
    try{
      const fb = window.__firebase;
      const ev = fb.API.ref(fb.db, 'donations');
      fb.API.onValue(ev,(snap)=>{
        try{ const dl = document.getElementById('diagListen'); if (dl) dl.textContent='attached'; }catch(_){}
        const val = snap.val() || {};
        cb(Object.values(val));
      }, (err)=>{
        const msg = (err && err.message) ? err.message : String(err);
        const diagE = document.getElementById('diagErr'); if (diagE) diagE.textContent = 'listen error: ' + msg;
      });
    }catch(err){
      const diagE = document.getElementById('diagErr'); if (diagE) diagE.textContent = 'list init error: ' + (err && err.message ? err.message : String(err));
    }
  }
